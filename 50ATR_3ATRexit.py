...
# Using .5ATR entry price WITH 3ATR EXIT
        ...
        for security in watchlist_dict['>'][mktcap]:
            ...
            if current_price < entry_price:
                if ticker not in get_all_portfolio_tickers():
                    ...
                    print(
                        f"BUYING RANGE..... {ticker}: Current Price: {current_price}, Entry Price: {entry_price}")
                    candle_object = SecurityTradeData(
                        ticker, num_of_periods=201)
                    atr = candle_object.df.iloc[-1][-1]

                    def main(ticker, sector, sector_allocation_dict, mktcap, daily_limit=25):
                        ...
                        if not long_cap and not daily_counter_cap:
                            if sector in sector_allocation_dict['long']['sector'].keys():
                                ...
                                sector_cap = False
                            if not sector_cap:
                                todays_trades = daily_closed_market_orders()
                                if ticker not in todays_trades and len(todays_trades) <= daily_limit:
                                    price = current_price
                                    if price == 0:
                                        ...
                                        if trailing_percentage < 25:
                                            ...
                                            print(
                                                f'{ticker} TRADE NOT PLACED... Trailing Percentage > 25%')
                                            return False, None, None, None, None
                                else:
                                    print(
        ...
        for security in watchlist_dict['<'][mktcap]:
            ...
            if current_price > entry_price:
                if ticker not in get_all_portfolio_tickers():
                    ...
                    sector = fetch_sector(ticker)
                    print(
                        f"SHORTING RANGE..... {ticker}: Current Price: {current_price}, Entry Price: {entry_price}")
                    candle_object = SecurityTradeData(
                        ticker, num_of_periods=201)
                    atr = candle_object.df.iloc[-1][-1]
...
if __name__ == '__main__':
    while True:
        ...
        position_atr_exit_scanner(exit_atr=3)
